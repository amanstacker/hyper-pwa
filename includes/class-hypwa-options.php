    ...
    final class HYPWA_Options {
        ...
        public static function sanitize( $input ) {
            ...
            foreach ( $text_toggle_fields as $field ) {
                if ( isset( $input[ $field ] ) ) {
                    $output[ $field ] = ( '1' == $input[ $field ] ) ? '1' : '0';
                }
            }
            ...
            if ( isset( $input['orientation'] ) ) {
                $output['orientation'] = in_array( $input['orientation'], $allowed_orientation, true ) ? $input['orientation'] : 'portrait';
            }

            $allowed_launch_mode = array( 'full-screen', 'standalone', 'minimal-ui', 'browser' );
            if ( isset( $input['launch_mode'] ) ) {
                $output['launch_mode'] = in_array( $input['launch_mode'], $allowed_launch_mode, true ) ? $input['launch_mode'] : 'standalone';
            }

            $allowed_status_bar = array( 'default', 'black', 'black-translucent' );
            if ( isset( $input['apple_status_bar_style'] ) ) {
                $output['apple_status_bar_style'] = in_array( $input['apple_status_bar_style'], $allowed_status_bar, true ) ? $input['apple_status_bar_style'] : 'default';
            }
